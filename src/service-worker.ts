...
const ASSETS = `cache${timestamp as number}`;
...
(self as any).addEventListener("activate", (event: ExtendableEvent) => {
  event.waitUntil(
    caches.keys().then(async (keys) => {
      ...
      return ((self as any) as ServiceWorkerGlobalScope).clients.claim();
    })
  );
});
...
async function fetchAndCache(request: Request) {
  const cache = await caches.open(`offline${timestamp as number}`);
